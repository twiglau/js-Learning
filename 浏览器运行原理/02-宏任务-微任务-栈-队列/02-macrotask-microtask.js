 ...
 * 在Firefox 39 和 Safari 8.0.7 却能按照像以上一样 "正确" 输出,这非常奇怪!
 */
 ...
 * 
 * 每个"线程"[thread]都有自己的事件循环[Event Loop],所以每个Web worker也都拥有自己的Event Loop,
 * 这样确保每个线程可独立地执行,因此同源的所有窗口共享一个Event Loop,确保他们之间可同步通信.
 * 
 * 事件循环[Event Loop]可持续地执行任何任务队列[Tasks Queued],一个Event Loop中有多种任务资源[Task Sources],Event Loop
 * 可确保这些资源(如IndexedDB等)顺序的执行, 浏览器可以能够从每个事件循环拿到一些资源来执行一个任务.这也
 * 使得浏览器对性能敏感型的任务,诸如用户输入等,能进行更好的处理.
 * 
 * 1. 宏任务[Tasks]是需调度的,因此浏览器可从其内部进入到JavaScript/DOM范围,并确保这些操作是按顺序进行的.
 * 在Tasks之间,浏览器可能需渲染更新. 从鼠标点击到事件回调需要任务调度,解析HTML[parsing HTML]需要,
 * setTimeout同样也需要.
 * 
 * setTimeout需一个延迟,然后才能为其回调[callback]调度一个新的任务. 这也是为什么 setTimeout 的输出晚于 script end.
 * 因此输出script end 是属于第一个任务[first task]的部分,而 setTimeout 的输出是一个独立的任务[separate task].
 * 
 * 2. Microtasks 通常被调度是在 当前执行脚本之后,这些内容应该直接发生的,诸如对一些动作做出反应,或在不新建全新任务的代价的情况下做些异步任务.
 * 只要没有其他JavaScript在执行中[mid-execution],并且在每个任务结束时,微任务队列就会在回调之后处理.
 * 在微任务期间排队的任何其他微任务都会添加到队列的末尾并进行处理.
 * 
 * Microtasks 包括 mutation observer 回调, promise 回调.
 * 
 * 一旦promise成立,或已经成立(settles),就会为其回调放入microtask队列,这确保promise回调是异步的,即使promise已经成立(settled).
 * 因此对已settled的promise调用 .then(yey,nay),会立即将微任务放入队列.
 * 
 * 这也是为什么promise1 和 promise2 的输出晚于 script end.
 * 因为在微任务(microtasks)被处理之前,当前运行脚本需执行完毕.
 * 
 * 而promise1 和 promise2 的输出在 setTimeout 之前, 是因为 microtasks 总是发生在下一个任务开始之前.
 * 所以,一步不输出,如下图:
 * 
 * 如图 02.1 视频
 */
 ...
 * 它们运行promise 回调晚于 setTimeout. 这可能它们视promise 回调(callbacks)为一个新的宏任务(task)而
 * 不是一个微任务(microtask)
 * 
 * 这点是有情可原的(sort-of excusable),因为promises来自ECMAScript而不是HTML.
 * ECMAScript具有类似于微任务的 "作业"[jobs]的概念,但除了模糊的邮件讨论之外,这种
 * 关系并不明确. 然而,普遍的共识(the general consensus)是 promise应该是 microtask 任务队列
 * 的一部分,这是有充分理由的.
 * 
 * 把promises视为宏任务(tasks)可能会导致性能可能,可能导致 回调被一些任务相关的事情(如渲染[rendering])所延迟,
 * 也可能导致回调的不确定性,由于与其他任务源的交互,并且可能会中断与其他API的交互.
 * 更多稍后会有介绍(but more on that later)
 * 
 * 这里是Edge浏览器把promises视作microtasks的票证。 [https://docs.microsoft.com/en-us/collaborate/connect-redirect]
 ...
 * 1. 调试, 通过调试输出来查看promises & setTimeout 的关系.
 ...
 * 3. 像上面提到的, 在ECMAScript场景中,将 microtasks 称作 "jobs",
 ...
 *   <div class="inner"></div>
 * </div>
 * 给出下面的JS代码,如果点击 div.inner 会输出什么?
 */
 ...
 * 如图 02.4
 * 
 * 在Chrome浏览器上能如上执行.关于一点需要指出的是:
 * > 微任务(microtasks)是在回调之后处理的(要要没有其他JavaScript在执行中),我认为
 * 这仅限于任务结束时. 此规则来自于调用回调的HTML规范(HTML spec):
